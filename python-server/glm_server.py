"""
GLM-4.7-Flash llama-cpp-python OpenAI-compatible server with tool calling support

Serves GLM-4.7-Flash model on /v1/chat/completions endpoint
...
async def lifespan(app: FastAPI):
    ...
    logger.info("=" * 60)
    logger.info("GLM-4.7-Flash llama-cpp-python Server Starting")
    logger.info("=" * 60)

    # Load Grammar
    grammar_path = Path(config.ROOT_DIR) / "python-server" / "glm_tools.gbnf"
...
app = FastAPI(title="GLM-4.7-Flash Server", lifespan=lifespan)
...
async def chat_completions(request: ChatCompletionRequest):
    ...
    try:
        ...
        # Explicit stop tokens for GLM-4.7/ChatML
        stop_tokens = [
            "<|im_end|>", 
            "<|endoftext|>", 
            "<|im_start|>user", 
...
def _auto_close_tool_xml(content: str) -> str:
    """
    Best-effort healing for partially emitted GLM XML tool blocks.
    """
    healed = content
